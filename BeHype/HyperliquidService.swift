...
class HyperliquidService: ObservableObject {
  ...
  func fetchExchangeData() {
    ...
    DispatchQueue.global(qos: .background).async {
      ...
      DispatchQueue.main.async {
        ...
        print(
          "💡 [DEBUG] Using direct API call for BTC price to work around getAllMids random sampling")
      ...
      DispatchQueue.main.async {
        ...
        if !candles.isEmpty {
          ...
          for (index, candle) in candles.prefix(3).enumerated() {
            print(
              "📊 [DEBUG] Candle \(index): Open=\(candle.open), Close=\(candle.close), High=\(candle.high), Low=\(candle.low)"
            )
          }
        } else {
  ...
  func placeLimitOrder(
    orderType: OrderType, amount: String, limitPrice: String,
    completion: @escaping (SwapResult) -> Void
  ) {
    print("📋 [DEBUG] Placing \(orderType == .buy ? "BUY" : "SELL") limit order...")
    print(
      "📋 [DEBUG] Amount: \(amount) \(orderType == .buy ? "USDC" : "BTC"), Price: $\(limitPrice)")
    ...
    guard let walletClient = walletClient else {
      completion(
        SwapResult(
          success: false, message: "❌ Wallet not loaded", orderId: nil, filledSize: nil,
          avgPrice: nil))
      return
    }
    ...
    if orderType == .buy && amount == "11.0" {
      ...
      DispatchQueue.global(qos: .background).async {
        ...
        print(
          "📏 [DEBUG] Price rounded from $\(limitPrice) to $\(roundedLimitPrice) for tick size compliance"
        )

        // Round BTC amount to 5 decimal places for precision compliance
        let roundedBtcAmount = self.roundBtcAmount(amount: amount)
        print(
          "📏 [DEBUG] BTC amount rounded from \(amount) to \(roundedBtcAmount) for precision compliance"
        )
  ...
  func placeBuyOrder(
    usdcAmount: String, limitPrice: String, completion: @escaping (SwapResult) -> Void
  ) {
    placeLimitOrder(
      orderType: .buy, amount: usdcAmount, limitPrice: limitPrice, completion: completion)
  }

  func placeSellOrder(
    btcAmount: String, limitPrice: String, completion: @escaping (SwapResult) -> Void
  ) {
    placeLimitOrder(
      orderType: .sell, amount: btcAmount, limitPrice: limitPrice, completion: completion)
  }
          ...
          do {
            if let fillsArray = try JSONSerialization.jsonObject(with: data) as? [[String: Any]] {
              ...
              if let firstFill = fills.first {
                print(
                  "📋 [DEBUG] Sample fill: \(firstFill.displayCoin) \(firstFill.displaySide) \(firstFill.sz) at \(firstFill.px)"
                )
              }
            } else {
