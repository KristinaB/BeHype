...
final class BeHypeUITests: XCTestCase {
  ...
  private func testHomeScreen(_ app: XCUIApplication) {
    ...
    // Verify main elements are present - navigation bar has no title
    XCTAssertTrue(app.navigationBars.element.exists, "Navigation bar should exist")
    
    // Verify branding text exists
    XCTAssertTrue(app.staticTexts["BeHype"].exists, "BeHype branding should exist")
    
    // Verify Fund Wallet button exists
    XCTAssertTrue(app.buttons["Fund Wallet"].exists, "Fund Wallet button should exist")
    ...
    XCTAssertTrue(
      app.staticTexts["Total Portfolio Value"].exists, "Total Portfolio Value card should exist")

    // Verify market data
    XCTAssertTrue(app.staticTexts["BTC/USDC"].exists, "BTC/USDC market should exist")
    
    // Verify View Chart button exists
    XCTAssertTrue(app.buttons["View Chart"].exists, "View Chart button should exist")
  }
  ...
  private func testTradeScreen(_ app: XCUIApplication) {
    ...
    XCTAssertTrue(
      app.navigationBars["Trade"].waitForExistence(timeout: 3), "Trade navigation should exist")
    ...
    XCTAssertTrue(
      hasEstimatedReceive || hasEstimatedValue,
  ...
  private func testTransactionsScreen(_ app: XCUIApplication) {
    ...
    XCTAssertTrue(
      app.navigationBars["Transactions"].waitForExistence(timeout: 3),
    ...
    XCTAssertTrue(
      app.textFields.containing(.staticText, identifier: "Search transactions...").element.exists,
  ...
  private func testChartFromHome(_ app: XCUIApplication) {
    ...
    XCTAssertTrue(
      app.navigationBars.element.waitForExistence(timeout: 3), "Home should be loaded")
    ...
    XCTAssertTrue(
      app.navigationBars["BTC/USDC Chart"].waitForExistence(timeout: 5), "Chart should open")
    ...
    XCTAssertTrue(
      app.navigationBars.element.waitForExistence(timeout: 3), "Should return to home")
  }
}
